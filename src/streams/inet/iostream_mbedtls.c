...
static ret_t tk_iostream_mbedtls_get_prop(object_t* obj, const char* name, value_t* v) {
  ...
  if (tk_str_eq(name, TK_STREAM_PROP_FD)) {
    ...
    bool_t is_ok1 = object_get_prop_bool(OBJECT(iostream_mbedtls->istream), TK_STREAM_PROP_IS_OK, TRUE);
    bool_t is_ok2 = object_get_prop_bool(OBJECT(iostream_mbedtls->ostream), TK_STREAM_PROP_IS_OK, TRUE);
...
static ret_t tk_iostream_mbedtls_on_destroy(object_t* obj) {
  ...
  socket_close(iostream_mbedtls->sock);
  object_unref(OBJECT(iostream_mbedtls->istream));
  object_unref(OBJECT(iostream_mbedtls->ostream));

  return RET_OK;
}

static const object_vtable_t s_tk_iostream_mbedtls_vtable = {.type = "tk_iostream_mbedtls",
...
tk_iostream_t* tk_iostream_mbedtls_create(int sock) {
  ...
  if (iostream_mbedtls == NULL) {
    socket_close(sock);
    return_value_if_fail(iostream_mbedtls != NULL, NULL);
  }

  iostream_mbedtls->sock = sock;
  iostream_mbedtls->istream = tk_istream_mbedtls_create(sock);
  iostream_mbedtls->ostream = tk_ostream_mbedtls_create(sock);
